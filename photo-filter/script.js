...
function inputChangeValue(event) {
    ...
    const measure = filterType === "hue-rotate" ? "deg" : filterType === "blur" ? "px" : "%";
    const filters = ctx.filter !== "none" ? ctx.filter.split(" ") : "blur(0px) invert(0) sepia(0) saturate(100%) hue-rotate(0)".split(" ");
    ...
    if (event.needToReset) input.value = (input.id === "input-saturate") ? 100 : 0;
...
function toggleFullscreen() {
    ...
    else document.getElementById("toggleFullscreen").requestFullscreen().catch(console.log);
}